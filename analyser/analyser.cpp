...
std::optional<CompilationError> Analyser::analyseMain() {
  ...
  auto var_declaration_err = analyseVariableDeclaration();
  if (var_declaration_err.has_value()) {
    return var_declaration_err;
  }
  auto stmts_err = analyseStatementSequence();
  if (stmts_err.has_value()) {
    return stmts_err;
  }
  // <变量声明>
...
std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
  ...
  while (true) {
    ...
    if (!next.has_value()) {
      return std::make_optional<CompilationError>(
          _current_pos, ErrorCode::ErrInvalidVariableDeclaration);
    }
    if (next.value().GetType() == TokenType::EQUAL_SIGN) {
      auto expression_err = analyseExpression();
      if (expression_err.has_value()) {
...
std::optional<CompilationError> Analyser::analyseConstantExpression(
  ...
  if (next.value().GetType() == TokenType::PLUS_SIGN 
    ...
    flag = (next.value().GetType() == TokenType::MINUS_SIGN) ? false : true;
    if (next.value().GetType() == TokenType::UNSIGNED_INTEGER) {
      ...
      try {
        out =
            std::any_cast<int32_t>(next.value().GetValue()) * (flag ? 1 : (-1));
        return std::nullopt;
      } catch (const std::bad_any_cast &) {
        return std::make_optional<CompilationError>(
            _current_pos, ErrorCode::ErrIntegerOverflow);
